# 📍 Dijital Görünürlük & Local SEO Case Study — Nail Salon (Manavgat/Side)

Bir güzellik salonunun internet üzerindeki dijital varlığını sıfırdan oluşturarak, markanın çevrimiçi görünürlüğünü artırmak için gerçekleştirdiğim kapsamlı bir dijital pazarlama projesidir. Google Maps, TripAdvisor ve Yandex üzerinde işletme profillerini oluşturup optimize ederek, hedeflenen tüm anahtar kelimelerde **Google'da 1. sıraya** taşıdım. Aynı zamanda Instagram'da Reels açıklamalarını SEO uyumlu hale getirerek, biyografiyi anahtar kelimelerle optimize ederek ve konum etiketlerini düzenleyerek aylık erişimi ciddi düzeyde artırdım.

> **Sonuç:** 70+ yorum · ⭐ 4.9 puan · 🏆 10+ anahtar kelimede Manavgat ve Side bölgesinde 1. sıra

---

## 📊 Sonuçlar

| Metrik | Değer |
|--------|-------|
| Google Maps Yorumları | 70+ |
| Google Maps Puanı | ⭐ 4.9 / 5.0 |
| Google 1. Sıra Anahtar Kelime Sayısı | 10+ (Manavgat & Side bölgesi) |
| Platformlar | Google Maps, TripAdvisor, Yandex, Instagram |
| Hedef Diller | Türkçe · İngilizce · Almanca · Rusça |

---

## 🔍 Anahtar Kelime Sıralamaları

### Türkçe
| Anahtar Kelime | Google Sırası |
|----------------|:------------:|
| manavgat tırnak | 🥇 1. sıra |
| manavgat nail art | 🥇 1. sıra |
| side tırnak | 🥇 1. sıra |
| side nail art | 🥇 1. sıra |

### Almanca
| Anahtar Kelime | Google Sırası |
|----------------|:------------:|
| manavgat nagelstudio | 🥇 1. sıra |
| side nagelstudio | 🥇 1. sıra |

### İngilizce
| Anahtar Kelime | Google Sırası |
|----------------|:------------:|
| manavgat nail salon | 🥇 1. sıra |
| side nail salon | 🥇 1. sıra |

### Rusça
| Anahtar Kelime | Google Sırası |
|----------------|:------------:|
| Манавгат маникюр | 🥇 1. sıra |
| Сиде маникюр | 🥇 1. sıra |

---

## 🛠 Yapılan Çalışmalar

### 1. Platform Kayıtları
- ✅ Google Business Profile oluşturuldu ve doğrulandı
- ✅ TripAdvisor işletme kaydı yapıldı ve doğrulandı
- ✅ Yandex Maps işletme kaydı yapıldı ve doğrulandı

### 2. Google Business Profile Optimizasyonu
- **Hizmetler:** SEO uyumlu, çok dilli tüm hizmetler detaylı açıklamalarla eklendi
- **Fotoğraflar:** Profesyonel salon ve çalışma fotoğrafları, dosya isimleri SEO uyumlu şekilde düzenlenerek yüklendi
- **Açıklama:** SEO uyumlu, çok dilli işletme açıklaması yazıldı
- **Kategoriler:** Ana ve alt kategoriler doğru şekilde seçildi
- **Çalışma Saatleri:** Güncel ve doğru tutuldu
- **Özellikler:** Randevu, ödeme yöntemleri vb. güncellendi

### 3. Yorum Stratejisi
- 📋 Google Maps yorum broşürü tasarlandı ve bastırıldı (QR kodlu)
- 💳 IBAN bilgileri için ayrı bir broşür tasarlandı
- 🪧 Broşürler pleksi standlarda salonda sergilendi
- 📈 Organik yorum sayısı 0 → 70+ arttı
- 💬 Yorumlara SEO uyumlu yanıtlar verildi
- 🌍 Gelen turistlerden kendi dillerinde Google Maps yorumu yazmaları sağlandı

### 4. Instagram SEO Optimizasyonu
- 📸 Reels açıklamaları SEO uyumlu şekilde düzenlendi
- 📝 Biyografi SEO'ya uygun anahtar kelimelerle optimize edildi
- 📍 Konum etiketleri doğru şekilde ayarlandı

### 5. Çok Dilli SEO
- Türkçe anahtar kelime optimizasyonu
- İngilizce anahtar kelime optimizasyonu (Uluslararası turist hedefli)
- Almanca anahtar kelime optimizasyonu (Alman turist hedefli)
- Rusça anahtar kelime optimizasyonu (Rus turist hedefli)

### 6. Broşür Tasarımı
- Modern ve profesyonel QR kodlu broşür tasarlandı
- Google Maps yorum sayfasına yönlendiren QR kod eklendi
- Bastırılarak salonda dağıtıma alındı

---

## 📸 Görseller

### Google Arama Sonuçları

<p align="left">
  <img src="https://github.com/user-attachments/assets/598e32b7-dfc9-4a04-ba17-3bad44caa4f2" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/0772a8d0-3dc8-4f30-91a4-2164c2cfec22" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/2d221712-ebcc-42be-8d0b-496aa4b764d5" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/f20442a6-5756-45f7-ad23-2328c0fae93e" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/d92a0ef0-8de8-4eb5-81b5-1b287f3acf3b" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/10dc7d84-2253-43da-b285-44a9f617907a" width="300"/>
</p>

### Google Maps Profil

<p align="left">
  <img src="https://github.com/user-attachments/assets/306b7fcf-c874-4a74-bea0-082cfee57b3f" width="400"/>
</p>

### Google Maps İşletme İstatistikleri

<p align="left">
  <img src="https://github.com/user-attachments/assets/0923fc23-27db-4b9a-a3dc-4cd3d1d776ee" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/f1ccf204-c414-435b-940f-2a69c02ea343" width="300"/>
</p>
<p align="left">
  <img src="https://github.com/user-attachments/assets/30574295-bd65-4254-91ff-52a0486322d9" width="300"/>
</p>

### Broşür Tasarımı

<p align="left">
  <img src="https://github.com/user-attachments/assets/12ee3b5e-b0a2-4fdd-abcb-81656672c4bd" width="400"/>
</p>

---

## 🧠 Öğrenilen Dersler

1. **Local SEO'da yorum sayısı ve kalitesi kritik** — Google, yüksek puanlı ve çok yorumlu işletmeleri ön plana çıkarıyor
2. **Çok dilli optimizasyon turist bölgelerinde fark yaratıyor** — Almanca ve Rusça anahtar kelimeler rekabetsiz ve kolay sıralama sağlıyor
3. **Fiziksel broşür + QR kod kombinasyonu etkili** — Müşteriye doğrudan ulaşmak dijital hatırlatmalardan daha etkili
4. **Düzenli güncelleme önemli** — Fotoğraf ve hizmet güncellemeleri Google'ın işletmeye verdiği önemi artırıyor

---

## 👤 Hakkımda

Bu proje, Manavgat'taki bir güzellik salonunun dijital pazarlama ihtiyaçlarını karşılamak üzere tarafımdan gerçekleştirilmiştir. Diğer projelerim için [GitHub profilime](https://github.com/yigitcanural) göz atabilirsiniz.
